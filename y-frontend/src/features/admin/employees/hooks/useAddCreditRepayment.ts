...
import type { CreditRepaymentFormValues } from "../schemas/employee";

export const useAddCreditRepayment = (employeeId?: string) => {
  const queryClient = useQueryClient();
  const { t } = useTranslation();

  return useMutation({
    mutationFn: (values: CreditRepaymentFormValues) => {
      ...
      return apiClient.post(
        `/employees/${employeeId}/account/credit-repayments`,
        payload,
      );
    },
    onSuccess: () => {
      toast.success(t("common:employee_credit_repayment_added_success"));
